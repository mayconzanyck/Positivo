...
select titulo, ano from filmes order by titulo, ano;
...
select titulo, ano from filmes where ano <= '2018' and '2022' order by ano, titulo;
...
select titulo, ano from filmes where ano between '2015' and '2018' order by ano, titulo;

/* Mostra todos os filmes lançados em 2015 ou 2018. */
select titulo, genero, ano from filmes where ano in ('2015' , '2018') order by ano, titulo; /mostra somente os especifiocs/
...
select titulo, genero, classificacao, ano from filmes where classificacao between '10' and '12' order by classificacao;


/* Mostra todos os filmes com classificação livre, 10 anos ou 12 anos. */
select titulo, genero, classificacao, ano from filmes where classificacao in ('Livre', '10', '12') order by classificacao;

/* Mostra todos os filmes com classificação diferente de livre, 10 anos ou 12 anos. */
select titulo, genero, classificacao, ano from filmes where classificacao not in ('Livre', '10', '12') order by classificacao;

/* Mostra os filmes de Romance lançados em 2006. */
select titulo, genero, ano from filmes where genero in ('Romance') and ano in ('2006') order by titulo, ano;
...
select titulo, genero, ano from filmes where genero in ('Animação', 'Terror') order by genero;


/* Mostra os filmes de Animação ou Terror. */
select titulo, genero, ano from filmes where genero = 'Animação' or genero = 'Terror' order by genero, ano desc;
...
select count(*) from filmes where genero in ('Romance');
...
select min(ano) from filmes where genero in ('Terror');
...
select genero, count(titulo) from filmes where classificacao = 'Livre' group by genero having count(titulo) >= 3;

/* Mostra os registros agrupados por ano e a contagem de registros por ano, classificados pela contagem. */
select ano, count() as Total_por_ano from filmes group by ano order by count();

/* Decrescente */
select ano, count() as Total_por_ano from filmes group by ano order by count() desc;

/* Mostra os registros agrupados por ano e a contagem de registros por ano, classificados pela contagem.
e que possuam 5 ou mais registros na contagem. */
select ano, count() as Total_por_ano from filmes group by ano  having count(ano) >=5 order by count();
